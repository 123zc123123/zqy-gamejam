...
| 文件 | 实际是什么 |
| --- | --- |
| `Prefabs/Canvas.prefab` | **大厅实际加载的页面**（缩放、底图、标题、棋盘都在这）。`Lobby` 加载 `Merge/Prefabs/Canvas`。 |
...
性格四选一（各 25%）：强攻（吕布）、灵巧（貂蝉）、智谋（诸葛亮）、耐战（关羽）。详情三行是名字、性格、品质。性格只加强势维，其余为 1。
