...
namespace DouQuqu
{
    ...
    public sealed class AISystem
    {
        ...
        private readonly float[] decisionTimers = new float[MatchController.MaxPlayers];
        private readonly float[] chargeTimers = new float[MatchController.MaxPlayers];
        private readonly float[] releaseAfter = new float[MatchController.MaxPlayers];
        private readonly bool[] charging = new bool[MatchController.MaxPlayers];
        ...
        public void Tick(MatchState state, InputFrame[] inputs, float dt)
        {
            ...
            MatchKnobs knobs = state.knobs ?? Rules.DefaultKnobs();
            dt = Mathf.Max(0f, dt);
            ...
            for (int i = 0; i < state.bugs.Length && i < inputs.Length; i++)
            {
                ...
                if (!Rules.CanStartCharge(knobs, bug)) continue;
        ...
        {
            float cap = Mathf.Min(
                Rules.EffectiveChargeTime(knobs, bug),
                Rules.StaminaChargeTCap(knobs, bug));
        ...
        private Vector2 KeepInsideDirection(MatchState state, BugState bug, Vector2 desired, MatchKnobs knobs)
        {
            ...
            Vector2 gradient = Rules.ArenaGradient(bug.position.x, bug.position.z);
        ...
        {
            if (maxDistance <= 0f || !Rules.InsideArena(bug.position)) return 0f;
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                if (Rules.InsideArena(point)) low = mid;
                else high = mid;
            }
            return low;
        }
        ...
        private PickupState FindNearestPickup(MatchState state, BugState source)
        {
            ...
            for (int i = 0; i < state.pickups.Count; i++)
            {
                ...
                if (pickup == null || !pickup.alive || !Rules.InsideArena(pickup.position)) continue;
