...
namespace DouQuqu
{
    ...
    public sealed class NestSystem
    {
        ...
        private void SpawnEggs(MatchState state, Vector3 center, Action<string, Vector3> emit)
        {
            ...
            float[] hatches = Rules.EggHatchTimes(count, state.knobs, random);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                for (int attempt = 0; attempt < 80; attempt++)
                {
                    ...
                    if (Rules.ArenaSdf(x, z) > -pad) continue;
                    bool packed = false;
                ...
                if (!placed) position = Rules.ClampInsideArena(center, pad);
        ...
        {
            float friction = Rules.FrictionAcceleration(state.knobs);
            for (int i = 0; i < state.eggs.Count; i++)
            {
                ...
                if (!Rules.InsideArena(egg.position))
                {
                    egg.alive = false;
