...
namespace DouQuqu
{
    ...
    public sealed class MatchClockHud : MonoBehaviour
    {
        ...
        private void Refresh(bool force)
        {
            ...
            MatchPhase phase = Rules.Phase(knobs, elapsed);
            bool rage = started && Rules.IsRage(knobs, elapsed);
            string clock = Rules.FormatClock(Rules.RemainingClock(knobs, elapsed, started));
            ...
            if (phase == MatchPhase.Over && started)
                valueText.text = Rules.FormatClock(0f);
        }
