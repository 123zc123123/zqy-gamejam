...
namespace DouQuqu
{
    ...
    public static class KnobSaveKeys
    {
    ...
    /// </summary>
    public sealed class KnobSaveHud : MonoBehaviour
    {
        private MatchController match;
        private TMP_Text label;
        private float resetLabelAt;

        public static void Ensure(MatchController match)
        {
            if (!Application.isEditor || !Application.isPlaying || match == null) return;
            if (FindObjectOfType<KnobSaveHud>() != null) return;

            RectTransform root = UiFactory.CreateOverlay("KnobSaveCanvas", 250);
            Button button = UiFactory.CreateButton(root, "SaveKnobsButton", "保存配置", null,
                new Vector2(0.72f, 0.91f), new Vector2(0.98f, 0.98f), Vector2.zero, Vector2.zero);
            KnobSaveHud hud = root.gameObject.AddComponent<KnobSaveHud>();
            hud.match = match;
