...
namespace DouQuqu
{
    ...
    public sealed class Lobby : MonoBehaviour
    {
        ...
        private BreedingBoardView breedingView;
        private CollectionController collection;
        private BattleEnterController battleEnter;
        private HeroSelectionController heroSelection;
        private ShopController shop;
        private RankingController ranking;
        private BottomNavBar nav;
        ...
        public static bool TryMapScene(string sceneName, out Page page)
        {
            ...
            if (sceneName == SceneNames.MainMenu) { page = Page.Home; return true; }
            if (sceneName == SceneNames.Merge) { page = Page.Merge; return true; }
            if (sceneName == SceneNames.Collection) { page = Page.Collection; return true; }
            if (sceneName == SceneNames.BattleEnter) { page = Page.BattleEnter; return true; }
            if (sceneName == SceneNames.HeroSelection) { page = Page.HeroSelection; return true; }
            if (sceneName == SceneNames.Shop) { page = Page.Shop; return true; }
            if (sceneName == SceneNames.Ranking) { page = Page.Ranking; return true; }
            return false;
        }
        ...
        public void ShowPage(Page page)
        {
            ...
            if (CurrentPage == Page.Merge && page != Page.Merge)
                PlayerDataService.CollectFinestFromBoard(GetComponent<MergeBoard>());
        ...
        private static bool TryMapNav(Page page, out BottomNavTab.NavModule module)
        {
            switch (page)
            {
                case Page.BattleEnter:
                    module = BottomNavTab.NavModule.Battle;
                    return true;
                case Page.Merge:
                    module = BottomNavTab.NavModule.Breeding;
                    return true;
                case Page.Collection:
                    module = BottomNavTab.NavModule.Registry;
                    return true;
                case Page.Shop:
                    module = BottomNavTab.NavModule.Shop;
                    return true;
                case Page.Ranking:
                    module = BottomNavTab.NavModule.Ranking;
                    return true;
                default:
                    module = BottomNavTab.NavModule.Battle;
                    return false;
        ...
        {
            GameObject home = FindNamed("MainMenu") ?? FindNamed("DouQuquMainMenu");
            if (home == null)
            {
                Debug.LogWarning("[DouQuqu] 大厅缺少村子主页 MainMenu");
                return;
            }

            Canvas canvas = home.GetComponentInParent<Canvas>();
            pages[(int)Page.Home] = canvas != null ? canvas.gameObject : home;
            BottomNavBar.SuppressEmbedded(pages[(int)Page.Home].transform);
        }

        private void EnsureRuntime()
        {
            MergeBoard board = GetComponent<MergeBoard>();
            if (board == null) board = gameObject.AddComponent<MergeBoard>();

            breedingView = GetComponent<BreedingBoardView>();
            if (breedingView == null) breedingView = gameObject.AddComponent<BreedingBoardView>();
            if (pages[(int)Page.Merge] != null) breedingView.AttachCanvas(pages[(int)Page.Merge]);

            if (GetComponent<MergeSceneController>() == null)
                gameObject.AddComponent<MergeSceneController>();

            collection = GetComponent<CollectionController>();
            if (collection == null) collection = gameObject.AddComponent<CollectionController>();
            if (pages[(int)Page.Collection] != null)
                TryBind("图鉴", () => collection.BindPage(pages[(int)Page.Collection]));

            battleEnter = GetComponent<BattleEnterController>();
            if (battleEnter == null) battleEnter = gameObject.AddComponent<BattleEnterController>();
            if (pages[(int)Page.BattleEnter] != null)
                TryBind("进战", () => battleEnter.BindPage(pages[(int)Page.BattleEnter]));

            heroSelection = GetComponent<HeroSelectionController>();
            if (heroSelection == null) heroSelection = gameObject.AddComponent<HeroSelectionController>();
            ...
            if (shop == null) shop = gameObject.AddComponent<ShopController>();
            if (pages[(int)Page.Shop] != null)
                TryBind("商店", () => shop.BindPage(pages[(int)Page.Shop]));

            ranking = GetComponent<RankingController>();
            if (ranking == null) ranking = gameObject.AddComponent<RankingController>();
        ...
        private void EnsureNav()
        {
            ...
            if (navCanvas == null)
            {
                RectTransform root = UiFactory.CreateOverlay("LobbyNavCanvas", 200);
                navCanvas = root.gameObject;
            }

            nav = BottomNavBar.EnsureOn(navCanvas.transform);
        ...
        private static GameObject Mount(string resourcesPath, string objectName)
        {
            ...
            BottomNavBar.SuppressEmbedded(instance.transform);
