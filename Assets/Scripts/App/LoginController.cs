    ...
    public sealed class LoginController : MonoBehaviour
    {
        ...
        private void BuildUi()
        {
            ...
            RectTransform panel = UiFactory.CreatePanel(root, "LoginPanel",
                new Vector2(0.08f, 0.27f), new Vector2(0.92f, 0.73f), Vector2.zero, Vector2.zero);
            UiFactory.CreateText(panel, "Title", "斗蟋蟀 Demo", 58f,
                new Vector2(0.08f, 0.72f), new Vector2(0.92f, 0.90f), Vector2.zero, Vector2.zero);
            UiFactory.CreateText(panel, "Hint", "输入名称后登录", 28f,
                new Vector2(0.08f, 0.59f), new Vector2(0.92f, 0.70f), Vector2.zero, Vector2.zero);
            nameInput = UiFactory.CreateInput(panel, "PlayerNameInput", "玩家名称",
                new Vector2(0.10f, 0.40f), new Vector2(0.90f, 0.54f), Vector2.zero, Vector2.zero);
            loginButton = UiFactory.CreateButton(panel, "LoginButton", "登录", Login,
                new Vector2(0.18f, 0.20f), new Vector2(0.82f, 0.34f), Vector2.zero, Vector2.zero);
            statusText = UiFactory.CreateText(panel, "Status", string.Empty, 24f,
                new Vector2(0.08f, 0.07f), new Vector2(0.92f, 0.16f), Vector2.zero, Vector2.zero);
