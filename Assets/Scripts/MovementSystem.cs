    ...
    public sealed class MovementSystem
    {
        ...
        public void TickCharge(MatchState state, InputFrame[] inputs, float dt)
        {
            for (int i = 0; i < state.bugs.Length; i++)
            {
                ...
                Rules.TickStamina(state.knobs, state.bugs[i], dt);
            }
        }
        ...
        public void MarkBabyOutOfBounds(MatchState state, Action<string, Vector3> emit)
        {
            for (int i = 0; i < state.babies.Count; i++)
            {
                ...
                if (!baby.alive || Rules.InsideArena(baby.position)) continue;
                baby.alive = false;
                baby.charging = false;
                baby.airborne = true;
        ...
        private void StepBugMotion(MatchState state, BugState bug, InputFrame input, float dt, Action<string, Vector3> emit, Action<BugState> onOut)
        {
            ...
            Rules.TickBuffs(state.knobs, bug, dt);
            ...
            if (!Rules.InsideArena(bug.position)) onOut?.Invoke(bug);
        }
        ...
        private void StepCharge(MatchKnobs knobs, BugState bug, InputFrame input, float dt)
        {
            ...
            float cap = Rules.StaminaChargeTCap(knobs, bug);
        ...
        private void IntegrateGroundOrAir(MatchKnobs knobs, BugState bug, float dt)
        {
            if (bug.airborne)
            {
                bug.verticalVelocity -= Rules.Gravity(knobs) * dt;
        ...
        private void StepBaby(MatchState state, BabyState baby, float dt, Action<string, Vector3> emit)
        {
            ...
            Rules.TickBuffs(state.knobs, baby, dt);
            ...
            if (baby.airborne)
            {
                baby.verticalVelocity -= Rules.Gravity(state.knobs) * dt;
            ...
            if (baby.attackCooldown <= 0f && !baby.airborne && IsBabySettled(baby))
            {
                ...
                baby.chargeTime = Mathf.Min(Rules.BabyChargeTime(state.knobs), baby.chargeTime + dt);
                if (baby.chargeTime >= Rules.BabyChargeTime(state.knobs) - 1e-4f)
        ...
        {
            float cost = Rules.JumpStaminaCost(knobs, bug);
            ...
            bug.verticalVelocity = speed * Rules.TanTheta(knobs);
            bug.height = 0.02f;
            bug.airborne = true;
        ...
        {
            float speed = Rules.BabyJumpSpeed(knobs, baby);
            ...
            Rules.SetLaunch(baby, baby.velocity);
            baby.verticalVelocity = speed * Rules.TanTheta(knobs);
            baby.height = 0.02f;
            baby.airborne = true;
            baby.slideMu = knobs.mu;
            baby.chargeTime = 0f;
            baby.charging = false;
            baby.attackCooldown = Rules.BabyAttackCooldown(knobs);
        }

        private bool BeginCharge(MatchKnobs knobs, BugState bug)
        {
            if (!Rules.CanStartCharge(knobs, bug)) return false;
            bug.charging = true;
            bug.chargeTime = 0f;
            bug.pendingCharge = false;
            return true;
        ...
        {
            return !bug.airborne && bug.height <= Epsilon && Rules.IsPlanarSettled(bug.velocity);
        }

        private bool IsBabySettled(BabyState baby)
        {
            return !baby.airborne && baby.height <= Epsilon && Rules.IsPlanarSettled(baby.velocity);
        }
        ...
        private void ApplyGroundFriction(MatchKnobs knobs, BugState bug, float dt)
        {
            ...
            float next = speed - Rules.FrictionAcceleration(knobs, bug.slideMu > 0f ? bug.slideMu : knobs.mu) * dt;
            if (next <= Rules.SettleSnap)
            {
                bug.velocity = Vector3.zero;
                Rules.ClearLaunch(bug);
                bug.hitTier = HitTier.None;
        ...
        private void ApplyBabyFriction(MatchKnobs knobs, BabyState baby, float dt)
        {
            ...
            if (speed <= Rules.SettleSnap)
            {
                ...
                Rules.ClearLaunch(baby);
                baby.hitTier = HitTier.None;
                return;
            }
            float mu = baby.slideMu > 0f ? baby.slideMu : knobs.mu;
            float next = speed - Rules.FrictionAcceleration(knobs, mu) * dt;
            if (next <= Rules.SettleSnap)
            {
                baby.velocity = Vector3.zero;
                Rules.ClearLaunch(baby);
