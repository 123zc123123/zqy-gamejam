...
namespace DouQuqu.Editor
{
    ...
    internal static class FontGenerator
    {
        ...
        private const string TargetPath = "Assets/Resources/Fonts/Chinese SDF.asset";

        // 先写入当前界面会用到的字。其余中文在运行时由动态图集补，不必一次打满字库。
        private const string DemoCharacters =
            "斗蟋蟀演示输入名称后登录成功主界面玩家按钮合成匹配图鉴退出返回局域网计时开始取消正在搜索等待槽位机器人进入游戏结束胜利你是本局赢家获胜者还没有前往将两只二级棋子到三级即可获得拖动它同等级棋盘满先空格完成抽卡结果操作最高级请将内松手必须是没有移动已取消分数最近生成并从消失保底进度按住蓄力合成完成获得道具当前等级点搜索秒成功剩余时间即将开放育虫盘图鉴活动排行商店学院金币规则背包放卵幼虫中虫成虫精品凡品灵品仙品极品强攻灵巧智谋沉稳耐战训练营六维介绍详情知道了重量抓地力上限回气刹稳滑出圈对局奖励" +
            "，。！？：；（）《》【】·…　/=-+＋%0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz点一下会先打完这句再进入下一新对白做成同目录的用调用把蛐蛐放槽里确定就能开打旁白队长样例对话框说话人揭完全组哦吼年轻人骨骼精奇好才地先借一只玩玩熟悉怎么了嘛听说村子中央超级厉害小铺谁开的回看看秋天农忙花太多时间玩乐所以我专门提供幼虫买各种物品只要给我时得到玉环就行有了去育虫庐培育点击返回你是金色红色绿色蓝色方饲料护罩沿饲主孵卵踢账号匹配组队撞开得分撞出对手留到最后区域即将崩塌皇冠立即装扮已拥有装饰孵化宝宝帮忙崩塌往里收口红圈闪金色万中无一专属学不去开眼更猛皮肤装饰样子强弱戴上对应积分击败正式比赛赢取榜与会豪杰排名";
