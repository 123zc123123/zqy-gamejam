...
namespace UnityFigmaBridge.Editor.Components
{
    public static class ComponentManager
    {
        ...
        public static void RemoveAllTemporaryNodeComponents(FigmaImportProcessData figmaImportProcessData)
       {
        ...
        {
            if (figmaImportProcessData.ComponentData.GetComponentPrefab(node.id) != null) return;
            ...
            var componentCount = figmaImportProcessData.ComponentData.GetComponentNameCount(node.id);
            var prefabAssetPath = FigmaPaths.GetPathForComponentPrefab(node, nodeName, componentCount);
            figmaImportProcessData.ComponentData.IncrementComponentNameCount(node.id,1);
