...
namespace UnityFigmaBridge.Editor.Utils
{
    public static class FigmaPaths
    {
        ...
        {
            return GetPathForComponentPrefab(nodeName, string.Empty, duplicateCount);
        }

        public static string GetPathForComponentPrefab(Node node, string displayName, int duplicateCount)
        {
            return GetPathForComponentPrefab(displayName, node != null ? node.id : string.Empty, duplicateCount);
        }

        public static string GetPathForComponentPrefab(string displayName, string nodeId, int duplicateCount)
        {
            var fileName = BuildPrefixedFileName(displayName, nodeId, duplicateCount);
            return $"{FigmaComponentPrefabFolder}/{fileName}.prefab";
        }
        
        public static string GetFileNameForNode(Node node,int duplicateCount)
        {
            return BuildPrefixedFileName(node != null ? node.name : "Node", node != null ? node.id : string.Empty, duplicateCount);
        }

        public static string BuildPrefixedFileName(string displayName, string nodeId, int duplicateCount)
        {
            var safeNodeTitle = ReplaceUnsafeCharacters(string.IsNullOrEmpty(displayName) ? "Node" : displayName);
            var idSuffix = FigmaDataUtils.ReplaceUnsafeFileCharactersForNodeId(nodeId ?? string.Empty);
            if (!string.IsNullOrEmpty(idSuffix))
            {
                safeNodeTitle += "_" + idSuffix;
            }
        ...
        public static void CreateRequiredDirectories(bool clearGeneratedPrefabs = true)
        {
