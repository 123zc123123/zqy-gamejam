...
namespace UnityFigmaBridge.Editor.FigmaApi
{
    ...
    public static class FigmaDataUtils
    {
        ...
        public static string GetNodeIdFromUrl(string url)
        {
            if (string.IsNullOrEmpty(url)) return string.Empty;
            const string marker = "node-id=";
            var index = url.IndexOf(marker, System.StringComparison.OrdinalIgnoreCase);
            if (index < 0) return string.Empty;
            var start = index + marker.Length;
            var end = url.IndexOf('&', start);
            var raw = end < 0 ? url.Substring(start) : url.Substring(start, end - start);
            ...
            {
                // Keep the raw value if it is already a node id.
            }
            return NormalizeNodeId(raw.Trim());
        }

        public static List<(Node Page, Node Node)> GetImportCandidateNodes(FigmaFile sourceFile)
        {
            var candidates = new List<(Node Page, Node Node)>();
            if (sourceFile?.document?.children == null) return candidates;
            foreach (var page in sourceFile.document.children)
            {
                CollectImportCandidates(page, page, candidates);
            }
            return candidates;
        }

        private static void CollectImportCandidates(Node page, Node node, List<(Node Page, Node Node)> candidates)
        {
            if (node?.children == null) return;
            foreach (var child in node.children)
            {
                ...
                {
                    CollectImportCandidates(page, child, candidates);
                    continue;
                }

                if (child.type == NodeType.FRAME || child.type == NodeType.COMPONENT ||
                    child.type == NodeType.COMPONENT_SET || child.type == NodeType.INSTANCE)
                {
                    candidates.Add((page, child));
                }
            }
        }

        public static HashSet<string> CollectRequiredComponentIds(IEnumerable<Node> selectedRoots,
            Dictionary<string, Node> lookup)
        {
            var required = new HashSet<string>();
            if (selectedRoots == null) return required;
            ...
            while (queue.Count > 0)
            {
                ...
                if (node.type == NodeType.INSTANCE && !string.IsNullOrEmpty(node.componentId))
                {
                    required.Add(node.componentId);
                    if (lookup != null && lookup.TryGetValue(node.componentId, out var definition) && definition != null)
                    {
        ...
        public static List<string> GetAllImageFillIdsFromNodes(IEnumerable<Node> rootNodes)
        {
            var imageFillIdList = new List<string>();
            if (rootNodes == null) return imageFillIdList;
            foreach (var root in rootNodes)
            {
                if (root == null) continue;
                GetAllImageFillIdsForNode(root, imageFillIdList, 2, true, root.type == NodeType.COMPONENT);
            }
            return imageFillIdList;
        }

        public static List<ServerRenderNodeData> FindAllServerRenderNodesForNodes(IEnumerable<Node> rootNodes,
            List<string> missingComponentIds)
        {
            var renderSubstitutionNodeList = new List<ServerRenderNodeData>();
            if (rootNodes == null) return renderSubstitutionNodeList;
            foreach (var root in rootNodes)
            {
                if (root == null) continue;
                // Start at depth 0 so selected screens with Figma export settings still
                // generate as UI hierarchies instead of a single server-rendered image.
                AddRenderSubstitutionsForFigmaNode(root, renderSubstitutionNodeList, 0, missingComponentIds, true,
                    root.type == NodeType.COMPONENT || root.type == NodeType.COMPONENT_SET);
            }
            return renderSubstitutionNodeList;
        }

        public static bool NodeTreeContainsAny(Node node, HashSet<string> nodeIds)
        {
            if (node == null || nodeIds == null || nodeIds.Count == 0) return false;
            if (nodeIds.Contains(node.id)) return true;
            if (node.children == null) return false;
            foreach (var child in node.children)
            {
                if (NodeTreeContainsAny(child, nodeIds)) return true;
            }
            return false;
        }
