...
namespace UnityFigmaBridge.Editor.Settings
{
    public class UnityFigmaBridgeSettings : ScriptableObject
    {
        ...
        [Tooltip("If true, import only the frames/components you tick. Required nested components are still generated.")]
        public bool OnlyImportSelectedFrames = true;

        [HideInInspector]
        public List<FigmaPageData> PageDataList = new ();

        [HideInInspector]
        public List<FigmaFrameData> FrameDataList = new ();
        ...
        public void RefreshForUpdatedFrames(FigmaFile file)
        {
            var candidates = FigmaDataUtils.GetImportCandidateNodes(file);
            var nextList = new List<FigmaFrameData>();
            foreach (var (page, node) in candidates)
            {
                var existing = FrameDataList.FirstOrDefault(f => f.NodeId == node.id);
                nextList.Add(new FigmaFrameData
                {
                    Name = node.name,
                    NodeId = node.id,
                    PageName = page != null ? page.name : string.Empty,
                    PageId = page != null ? page.id : string.Empty,
                    NodeType = node.type.ToString(),
                    Selected = existing != null && existing.Selected
                });
            }

            FrameDataList = nextList;
        }

        public List<string> GetSelectedFrameIds()
        {
            return FrameDataList.Where(f => f.Selected).Select(f => f.NodeId).ToList();
        }

        public bool TrySelectFrameByNodeId(string nodeId)
        {
            var normalized = FigmaDataUtils.NormalizeNodeId(nodeId);
            if (string.IsNullOrEmpty(normalized)) return false;
            var match = FrameDataList.FirstOrDefault(f =>
                f.NodeId == normalized || FigmaDataUtils.NormalizeNodeId(f.NodeId) == normalized);
            if (match == null) return false;
            foreach (var frame in FrameDataList)
            {
                frame.Selected = frame == match;
            }
            return true;
        }
    }
