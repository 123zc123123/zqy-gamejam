    ...
    public static class FigmaAssetGenerator
    {
        ...
        public static void BuildFigmaFile(Canvas rootCanvas, FigmaImportProcessData figmaImportProcessData)
        {
            ...
            foreach (var figmaCanvasNode in figmaImportProcessData.SourceFile.document.children)
            {
                ...
                if (figmaImportProcessData.RestrictToSelectedNodes &&
                    !FigmaDataUtils.NodeTreeContainsAny(figmaCanvasNode, figmaImportProcessData.SelectedNodeIds) &&
                    !FigmaDataUtils.NodeTreeContainsAny(figmaCanvasNode, figmaImportProcessData.RequiredComponentIds))
                {
                    continue;
                }
            ...
            {
                if (figmaImportProcessData.RestrictToSelectedNodes) continue;
            ...
            FigmaImportProcessData figmaImportProcessData, bool includedPageObject)
        {
            ...
            if (pageNode.children != null)
            {
                foreach (var childNode in pageNode.children)
                {
                    if (figmaImportProcessData.RestrictToSelectedNodes)
                    {
                        GenerateRestrictedNodes(childNode, pageTransform, pageNode, 0, figmaImportProcessData);
                        continue;
                    }
        ...
        private static void GenerateRestrictedNodes(Node node, RectTransform parentTransform, Node parentFigmaNode,
            int nodeRecursionDepth, FigmaImportProcessData figmaImportProcessData)
        {
            if (node == null || !CheckNodeValidForGeneration(node, figmaImportProcessData)) return;

            if (figmaImportProcessData.SelectedNodeIds.Contains(node.id))
            {
                BuildFigmaNode(node, parentTransform, parentFigmaNode, 0, figmaImportProcessData, true, false);
                return;
            }

            var isRequiredComponent = figmaImportProcessData.RequiredComponentIds.Contains(node.id) &&
                                      (node.type == NodeType.COMPONENT || node.type == NodeType.COMPONENT_SET);
            if (isRequiredComponent)
            {
                BuildFigmaNode(node, parentTransform, parentFigmaNode, nodeRecursionDepth, figmaImportProcessData, false, false);
                return;
            }
            ...
            {
                if (FigmaDataUtils.NodeTreeContainsAny(child, figmaImportProcessData.SelectedNodeIds) ||
                    FigmaDataUtils.NodeTreeContainsAny(child, figmaImportProcessData.RequiredComponentIds))
                {
                    GenerateRestrictedNodes(child, parentTransform, node, nodeRecursionDepth + 1, figmaImportProcessData);
                }
            }
        }
        ...
        {
            if (figmaImportProcessData.RestrictToSelectedNodes &&
                figmaImportProcessData.SelectedNodeIds != null &&
                figmaImportProcessData.SelectedNodeIds.Contains(node.id))
            {
                return true;
            }
            ...
            int nodeRecursionDepth, FigmaImportProcessData figmaImportProcessData,bool includedPageObject, bool withinComponentDefinition)
        {
            ...
            switch (figmaNode.type)
            {
                ...
                case NodeType.FRAME:
                    var saveAsScreen = figmaImportProcessData.RestrictToSelectedNodes
                        ? figmaImportProcessData.SelectedNodeIds.Contains(figmaNode.id)
                        : includedPageObject && FigmaDataUtils.IsScreenNode(figmaNode,parentFigmaNode);
                    if (saveAsScreen)
                    {
