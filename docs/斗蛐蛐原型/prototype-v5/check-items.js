...

{
  ...
  approx("baby A1", stats.vRate, 23 * 0.7, 1e-9);
  approx("baby tMax", stats.tMax, 0.16, 1e-9);
  const grownBaby = { grow: 2, chargeT: 0.16 };
  approx("baby grow A1", R.babyChargeStats(knobs, grownBaby).vRate, 23 * 0.7 * g, 1e-9);
  approx("baby grow Δv", R.babyChargeDeltaV(knobs, grownBaby), 23 * 0.7 * g * 0.16, 1e-9);
